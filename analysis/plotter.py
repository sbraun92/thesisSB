...
class Plotter(object):
    def __init__(self, path, it, rewardPlot=True, stateExpPlot=True, stepPlot=True, epsHistory=True, algorithm=None,
                 smoothing_window=None, plot_standard_dev=True):
    ...
    def plotRewardPlot(self, totalRewards):
        ...
        rewards_smoothed = pd.Series(totalRewards).rolling(self.smoothing_window,
                                                           min_periods=self.smoothing_window).mean()

        if self.plot_standard_dev:
            rewards_q75 = rewards_smoothed + pd.Series(totalRewards).rolling(self.smoothing_window,
                                                                         min_periods=self.smoothing_window).std()
            rewards_q25 = rewards_smoothed - pd.Series(totalRewards).rolling(self.smoothing_window,
                                                                         min_periods=self.smoothing_window).std()
    ...
    def plotStepPlot(self, stepsToExit):
        ...
        if self.algorithm is not None:
            plt.title(self.algorithm + ": Steps to Finish over Time (Smoothed over window size {})".format(
                int(self.smoothing_window / 2)))
            fi4.savefig(self.path + self.algorithm + '_StepsToFinish.pdf', dpi=600, bbox_inches="tight")
        else:
            plt.title("Steps to Finish over Time (Smoothed over window size {})".format(int(self.smoothing_window / 2)))
            fi4.savefig(self.path + 'StepsToFinish.pdf', dpi=600, bbox_inches="tight")
        logging.getLogger('log1').info("Finished plot")
