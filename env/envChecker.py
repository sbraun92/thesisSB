    ...
    def check_input_consistency(self):
        ...
        if self.env.rows < self.env.lanes:
            logging.getLogger(__name__).warning(
                "Unusual deck dimensions: More lanes than rows ... might not be fatal " +
                "thus ignore it")
        ...
        if 2 * self.env.hull_catheti_length > self.env.rows:
            error_msg = 'Too much hull_catheti_length (was {}): Double hull_catheti_length ({})' + \
                        ' is more than Rows ({})' + \
                        '-> Should be less or equal'.format(self.env.hull_catheti_length,
                                                            self.env.hull_catheti_length * 2, self.env.lanes)
            logging.getLogger(__name__).error(error_msg)
        ...
        if not isinstance(self.env.vehicle_data, np.ndarray):
            error_msg = 'vehicle_data argument was of type {} but must be a numpy array'.format(
                type(self.env.vehicle_data))
            logging.getLogger(__name__).error(error_msg)
            raise TypeError(error_msg)

        if not all(isinstance(x, np.integer) for x in self.env.vehicle_data.flatten()):
            error_msg = 'Not all elements of vehicle_data have been of type np.integer -' + \
                        ' Please ensure by using e.g. dtype=np.int'
            logging.getLogger(__name__).error(error_msg)
            raise TypeError(error_msg)

        if len(self.env.vehicle_data[0]) != len(np.unique(self.env.vehicle_data[0])):
            error_msg = 'The first row of vehicle_data represents the action id -' + \
                        ' therefore each element should be unique'
            logging.getLogger(__name__).error(error_msg)
